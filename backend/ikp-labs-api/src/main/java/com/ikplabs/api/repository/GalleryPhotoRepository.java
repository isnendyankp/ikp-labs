 ...
 * - findById() → Cari foto by ID
 * - save() → Simpan foto baru atau update
 * - delete() → Hapus foto
...
public interface GalleryPhotoRepository extends JpaRepository<GalleryPhoto, Long> {
     ...
     * Request page 0, size 20 → Returns first 20 photos
     * Request page 1, size 20 → Returns last 5 photos
     *
     ...
     * Example: /api/gallery/public/83 → Show User 83's public photos
     *
     * Filters:
     * 1. user_id = {userId}
     * 2. is_public = TRUE
     ...
     *    - findBy{Field} → Spring generate query otomatis
     *    - findByUserId → WHERE user_id = ?
     *    - findByIsPublicTrue → WHERE is_public = TRUE
     *    - findByUserIdAndIsPublicTrue → WHERE user_id = ? AND is_public = TRUE
     *
     * 4. Count Methods:
     ...
     *
     * 5. Privacy Filtering:
     *    - findByUserId → Returns ALL (public + private) for owner
     *    - findByIsPublicTrue → Returns ONLY public for everyone
     *    - findByUserIdAndIsPublicTrue → Returns ONLY user's public for others
     *
     ...
     *    - User deleted → All photos automatically deleted
     *    - No need for manual cleanup method
     *
     * 8. Index Usage:
     *    - user_id indexed → Fast findByUserId queries
     *    - is_public indexed → Fast findByIsPublicTrue queries
     *    - Both → Fast compound queries
