...
test.describe("Desktop Viewport - 1920x1080 (Full HD)", () => {
  ...
  test("Should display landing page correctly on Full HD", async ({ page }) => {
    ...
    await expect(
      navbar.getByRole("button", { name: "Features", exact: true }),
    ).toBeVisible();
    await expect(
      navbar.getByRole("button", { name: "About", exact: true }),
    ).toBeVisible();
    await expect(
      navbar.getByRole("button", { name: "Login", exact: true }),
    ).toBeVisible();
    await expect(
      navbar.getByRole("button", { name: "Get Started", exact: true }),
    ).toBeVisible();
  ...
  test("Should navigate to register when clicking Get Started on Full HD", async ({
    page,
  }) => {
  ...
  test("Should display features section in 3-column layout on Full HD", async ({
    page,
  }) => {
  ...
  test("Should display footer in 4-column layout on Full HD", async ({
    page,
  }) => {
    ...
    await expect(
      footer.getByRole("heading", { name: "Kameravue" }),
    ).toBeVisible();
  ...
  test("Should show Go to Gallery button when authenticated on Full HD", async ({
    page,
  }) => {
  ...
  test("Should display login page correctly on Full HD", async ({ page }) => {
    // FIXME: Login page element assertions fail in CI (works locally)
    test.fixme();
    ...
    await expect(
      page.getByRole("link", { name: "Don't have an account? Sign up" }),
    ).toBeVisible();
  ...
  test("Should display register page correctly on Full HD", async ({
    page,
  }) => {
    // FIXME: Register page element assertions fail in CI (works locally)
    test.fixme();
  ...
  test("Should display gallery page correctly on Full HD", async ({ page }) => {
    ...
    const mobileControls = page.locator(
      "div.flex.items-center.gap-1.sm\\.hidden",
    );
...
test.describe("Desktop Viewport - 1366x768 (Laptop)", () => {
  ...
  test("Should display landing page correctly on laptop resolution", async ({
    page,
  }) => {
  ...
  test("Should display gallery correctly on laptop resolution", async ({
    page,
  }) => {
...
test.describe("Desktop Viewport - 1280x720 (HD)", () => {
  ...
  test("Should display landing page correctly on HD resolution", async ({
    page,
  }) => {
