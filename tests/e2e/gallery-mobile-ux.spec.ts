...
test.describe("Mobile UX Improvements", () => {
  ...
  test.describe("Mobile Header Controls", () => {
    test("E2E-MOBILE-001: filter icon visible on mobile", async ({ page }) => {
      ...
      console.log(
        "✅ E2E-MOBILE-001: Filter icon visible on mobile test PASSED",
      );
    });
    ...
    test("E2E-MOBILE-003: filter dropdown opens when tapped", async ({
      page,
    }) => {
    ...
    test("E2E-MOBILE-004: sort dropdown opens when tapped", async ({
      page,
    }) => {
      // GIVEN: User is logged in and viewing gallery on mobile
      const { user } = await createAuthenticatedGalleryUser(page);
  ...
  test.describe("FAB Upload Button", () => {
    test("E2E-MOBILE-005: FAB button visible on all devices", async ({
      page,
    }) => {
      // GIVEN: User is logged in
      const { user } = await createAuthenticatedGalleryUser(page);
    ...
    test("E2E-MOBILE-006: FAB button navigates to upload page", async ({
      page,
    }) => {
    ...
    test("E2E-MOBILE-007: FAB button positioned bottom-right", async ({
      page,
    }) => {
      // GIVEN: User is logged in and viewing gallery
      const { user } = await createAuthenticatedGalleryUser(page);
  ...
  test.describe("Back to Top Button", () => {
    test("E2E-MOBILE-009: back to top appears after scrolling", async ({
      page,
      browserName,
    }) => {
      test.skip(
        browserName === "firefox",
        "Scroll event handling differs in Firefox test environment",
      );
      test.skip(
        browserName === "chromium",
        "Test is flaky due to timing issues; functionality covered by other tests",
      );
    ...
    test("E2E-MOBILE-010: back to top hidden at top of page", async ({
      page,
    }) => {
      // GIVEN: User is logged in and viewing gallery
      const { user } = await createAuthenticatedGalleryUser(page);
    ...
    test("E2E-MOBILE-011: back to top scrolls to top smoothly", async ({
      page,
      browserName,
    }) => {
      test.skip(
      ...
      const backToTop = page.getByLabel("Scroll to top");
      await page.waitForSelector('[aria-label="Scroll to top"]', {
        timeout: 5000,
      });
    ...
    test("E2E-MOBILE-012: back to top positioned correctly", async ({
      page,
      browserName,
    }) => {
      // FIXME: Back to top button position assertion fails in CI (scroll event timing)
      test.fixme();
      test.skip(
        browserName === "firefox",
        "Scroll event handling differs in Firefox test environment",
  ...
  test.describe("Desktop Sticky Controls", () => {
    ...
    test("E2E-MOBILE-014: controls accessible when scrolled", async ({
      page,
    }) => {
      // GIVEN: User is logged in and viewing gallery on desktop
      const { user } = await createAuthenticatedGalleryUser(page);
  ...
  test.describe("Cross-Device Layout", () => {
    test("E2E-MOBILE-017: mobile layout correct (< 640px)", async ({
      page,
    }) => {
      // GIVEN: User is logged in
      const { user } = await createAuthenticatedGalleryUser(page);
      ...
      const desktopFilterButton = page
        .locator("button")
        .filter({ hasText: "All Photos" });
    ...
    test("E2E-MOBILE-018: desktop layout correct (>= 640px)", async ({
      page,
    }) => {
      // GIVEN: User is logged in
      const { user } = await createAuthenticatedGalleryUser(page);
    ...
    test("E2E-MOBILE-019: tablet layout correct (640-1024px)", async ({
      page,
    }) => {
      // GIVEN: User is logged in
      const { user } = await createAuthenticatedGalleryUser(page);
  ...
  test.describe("Feature Parity", () => {
    test("E2E-MOBILE-021: all filter options work on mobile", async ({
      page,
    }) => {
      // GIVEN: User is logged in and viewing gallery on mobile
      const { user } = await createAuthenticatedGalleryUser(page);
    ...
    test("E2E-MOBILE-022: all sort options work on mobile", async ({
      page,
    }) => {
      // GIVEN: User is logged in and viewing gallery on mobile
      const { user } = await createAuthenticatedGalleryUser(page);
  ...
  test.describe("Scroll Position Restoration", () => {
    test("E2E-MOBILE-025: scroll position saved when clicking photo", async ({
      page,
    }) => {
      // GIVEN: User is logged in and has scrolled down
      const { user } = await createAuthenticatedGalleryUser(page);
    ...
    test("E2E-MOBILE-026: scroll position restored when returning to gallery", async ({
      page,
      browserName,
    }) => {
      test.skip(
        browserName === "firefox",
        "Scroll position restoration differs in Firefox test environment",
      );
    ...
    test("E2E-MOBILE-027: scroll restoration works on mobile", async ({
      page,
      browserName,
    }) => {
      test.skip(
    ...
    test("E2E-MOBILE-028: scroll restoration works on desktop", async ({
      page,
    }) => {
