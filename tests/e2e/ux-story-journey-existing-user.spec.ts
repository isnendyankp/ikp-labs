...
test.describe("UX Story Journey", () => {
  ...
  test("Filter State Bug Fix Demo (with existing photos)", async ({ page }) => {
    // FIXME: Logs in as demo@kameravue.com which doesn't exist in CI fresh database
    test.fixme();
    // ========== PART 1: LOGIN ==========
    ...
    await test.step("Test sort filter state preservation", async () => {
      ...
      const uploadLink = page
        .locator('a:has-text("Upload"), button:has-text("Upload Photo")')
        .first();
      ...
      const backButton = page
        .locator('button:has-text("Back"), a:has-text("Back"), text=←')
        .first();
      ...
      console.log(
        "✅ Bug Fix Verified: Sort state preserved after navigation!",
      );
    });
