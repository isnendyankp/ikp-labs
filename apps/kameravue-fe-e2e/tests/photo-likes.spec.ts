...
test.describe("Photo Likes Feature", () => {
  ...
  test.describe("Basic Like/Unlike Operations", () => {
    test("E2E-PL-001: should like photo from gallery view", async ({
      page,
    }) => {
      ...
      const photoCard = page
        .locator('h3:has-text("Photo to Like")')
        .locator("../..")
        .first();
      ...
      if (urlAfterClick.includes("/gallery/photo/")) {
        throw new Error(
          "❌ Like button click navigated to detail page! Selector is wrong.",
        );
      }

      // THEN: Like count increases to 1
      // Re-query elements after React re-render to avoid stale element references
      const updatedPhotoCard = page
        .locator('h3:has-text("Photo to Like")')
        .locator("../..");
      const updatedLikeButton = updatedPhotoCard
        .locator("button")
    ...
    test("E2E-PL-002: should unlike photo from gallery view", async ({
      page,
    }) => {
      // FIXME: Photo upload timeout in CI - upload flow too slow on GitHub Actions runners
      test.fixme();
      ...
      const photoCard = page
        .locator('h3:has-text("Photo to Unlike")')
        .locator("../..");
      const likeButton = photoCard
        .locator("button")
        .filter({ has: page.locator("svg") })
        .first();
      const likeButtonWrapper = likeButton.locator(".."); // Parent div contains button + count
    ...
    test("E2E-PL-003: should like photo from detail view", async ({ page }) => {
      ...
      const photoCard = page
        .locator('h3:has-text("Detail Like Photo")')
        .locator("../..")
        .first();
      const galleryLikeButton = photoCard
        .locator("button")
        .filter({ has: page.locator("svg") })
        .first();
  ...
  test.describe("Persistence Tests", () => {
    test("E2E-PL-004: like persists after page refresh", async ({ page }) => {
      ...
      const refreshedPhotoCard = page
        ...
        .locator("../..")
        .first();
      const refreshedLikeButton = refreshedPhotoCard
        .locator("button")
        .filter({ has: page.locator("svg") })
        .first();
      ...
      console.log(
        "✅ E2E-PL-004: Like persists after page refresh test PASSED",
      );
    });

    test("E2E-PL-005: like persists after logout and login", async ({
      page,
    }) => {
      // FIXME: Photo upload timeout in CI - upload flow too slow on GitHub Actions runners
      test.fixme();
      ...
      const persistedPhotoCard = page
        ...
        .locator("../..")
        .first();
      const persistedLikeButton = persistedPhotoCard
        .locator("button")
        .filter({ has: page.locator("svg") })
        .first();
      ...
      console.log(
        "✅ E2E-PL-005: Like persists after logout and login test PASSED",
      );
    });
  });

  test.describe("Multi-User Tests", () => {
    test("E2E-PL-006: multiple users can like same photo", async ({
      page,
      context,
      browser,
    }) => {
      ...
      const photoCardB = page
        .locator('h3:has-text("Multi-User Like Photo")')
        .locator("../..");
      const likeButtonB = photoCardB
        .locator("button")
        .filter({ has: page.locator("svg") })
        .first();
      ...
      const photoCardC = pageC
        .locator('h3:has-text("Multi-User Like Photo")')
        .locator("../..");
      const likeButtonC = photoCardC
        .locator("button")
        .filter({ has: pageC.locator("svg") })
        .first();
      ...
      const refreshedPhotoCardB = page
        .locator('h3:has-text("Multi-User Like Photo")')
        .locator("../..");
      const refreshedLikeButtonB = refreshedPhotoCardB
        .locator("button")
        .filter({ has: page.locator("svg") })
        .first();
      const refreshedLikeButtonWrapperB = refreshedLikeButtonB.locator("..");

      const refreshedLikeTextB =
        await refreshedLikeButtonWrapperB.textContent();
      ...
      console.log(
        "✅ E2E-PL-006: Multiple users can like same photo test PASSED",
      );
    });
  });
  ...
  test.describe("Liked Photos Page", () => {
    test("E2E-PL-007: view liked photos page", async ({ page }) => {
      ...
      const photo1Card = page
        .locator('h3:has-text("Liked Photo 1")')
        .locator("../..");
      const likeButton1 = photo1Card
        .locator("button")
        .filter({ has: page.locator("svg") })
        .first();
      ...
      const photo2Card = page
        .locator('h3:has-text("Liked Photo 2")')
        .locator("../..");
      const likeButton2 = photo2Card
        .locator("button")
        .filter({ has: page.locator("svg") })
        .first();
    ...
    test("E2E-PL-008: unlike from liked photos page", async ({ page }) => {
      ...
      const photoCard = page
        .locator('h3:has-text("Photo to Unlike from Liked Page")')
        .locator("../..");
      const likeButton = photoCard
        .locator("button")
        .filter({ has: page.locator("svg") })
      ...
      const likedPhoto = page.locator(
        'h3:has-text("Photo to Unlike from Liked Page")',
      );
      await expect(likedPhoto).toBeVisible();

      // WHEN: User unlikes the photo from liked photos page
      const likedPhotoCard = page
        .locator('h3:has-text("Photo to Unlike from Liked Page")')
        .locator("../..");
      const unlikeButton = likedPhotoCard
        .locator("button")
        .filter({ has: page.locator("svg") })
        .first();
    ...
    test("E2E-PL-009: empty state when no liked photos", async ({ page }) => {
      ...
      console.log(
        "✅ E2E-PL-009: Empty state when no liked photos test PASSED",
      );
    });
  });
});
